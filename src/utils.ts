...
export const cookieOptFilter = (cookieOpt: LibCurlGetCookiesOption) => {
    ...
    return (e: LibCurlCookieAttrArray) => {
        if (domainArr && !domainArr.find((t) => e[0] === t))
            //设置了domain 找上级所有域名cookie 没设置就不过滤
            return false;
        if (cookieOpt?.path) {
...
export const libcurlRandomJA3Fingerprint: () => string = () => {
    ...
    const extensions =
        "0-23-65281-10-11-35-16-5-13-18-51-45-43-27-17613-65037".split("-");
    const randomBytes = crypto.getRandomValues(
        new Uint32Array(extensions.length),
    );
    for (let i = randomBytes.length - 1; i >= 0; i--) {
        const pos = randomBytes[i] % (i + 1);
        [extensions[i], extensions[pos]] = [extensions[pos], extensions[i]];
    }
    return `771,4865-4866-4867-49195-49199-49196-49200-52393-52392-49171-49172-156-157-47-53,${extensions.join("-")}-41,4588-29-23-24,0`;
};
