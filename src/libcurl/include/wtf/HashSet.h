...
namespace WTF {
	...
	{
		return CaseFoldingHashReader<T>().ReadSmall(&ch, 2);
	}

	inline unsigned stringHash(std::string key)
	{
		auto a = WTF::StringHasher::ComputeHashAndMaskTop8Bits <LChar, foldCase<LChar>> (reinterpret_cast<const LChar*>(key.data()), key.size());
		return a;
	}
	...
	struct StringHash {
		static unsigned hash(std::string key) {
	...
	struct PtrHashBase<T, true /* isSmartPtr */> {
		...
		static bool equal(PtrType a, const T& b) {
			return a == getPtr(b);
		}
		static bool equal(const T& a, PtrType b) {
			return getPtr(a) == b;
		}
	};
	...
	template<typename P, typename Deleter> struct DefaultHash<std::unique_ptr<P, Deleter>> {
		typedef PtrHash<std::unique_ptr<P, Deleter>> Hash;
	};

	// make IntPairHash the default hash function for pairs of (at most) 32-bit integers.

	template<> struct DefaultHash<std::pair<short, short>> {
		typedef IntPairHash<short, short> Hash;
	};
	template<> struct DefaultHash<std::pair<short, unsigned short>> {
		typedef IntPairHash<short, unsigned short> Hash;
	};
	template<> struct DefaultHash<std::pair<short, int>> {
		typedef IntPairHash<short, int> Hash;
	};
	template<> struct DefaultHash<std::pair<short, unsigned>> {
		typedef IntPairHash<short, unsigned> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned short, short>> {
		typedef IntPairHash<unsigned short, short> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned short, unsigned short>> {
		typedef IntPairHash<unsigned short, unsigned short> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned short, int>> {
		typedef IntPairHash<unsigned short, int> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned short, unsigned>> {
		typedef IntPairHash<unsigned short, unsigned> Hash;
	};
	template<> struct DefaultHash<std::pair<int, short>> {
		typedef IntPairHash<int, short> Hash;
	};
	template<> struct DefaultHash<std::pair<int, unsigned short>> {
		typedef IntPairHash<int, unsigned short> Hash;
	};
	template<> struct DefaultHash<std::pair<int, int>> {
		typedef IntPairHash<int, int> Hash;
	};
	template<> struct DefaultHash<std::pair<int, unsigned>> {
		typedef IntPairHash<unsigned, unsigned> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned, short>> {
		typedef IntPairHash<unsigned, short> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned, unsigned short>> {
		typedef IntPairHash<unsigned, unsigned short> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned, int>> {
		typedef IntPairHash<unsigned, int> Hash;
	};
	template<> struct DefaultHash<std::pair<unsigned, unsigned>> {
		typedef IntPairHash<unsigned, unsigned> Hash;
	};
	...
	template<typename... Types> struct DefaultHash<std::tuple<Types...>> {
		typedef TupleHash<Types...> Hash;
	};
	...
	template<typename T> struct GenericHashTraits : GenericHashTraitsBase<std::is_integral<T>::value, T> {
		...
		typedef T PeekType;
		template<typename U> static U&& peek(U&& value) {
			return std::forward<U>(value);
		}

		typedef T TakeType;
		template<typename U> static TakeType take(U&& value) {
			return std::forward<U>(value);
		}
	};
	...
	struct StringHashTraits : GenericHashTraits<std::string> {
		static bool isDeletedValue(std::string value) {
			return false;
		}
	};
	...
	template<typename T> struct SimpleClassHashTraits : GenericHashTraits<T> {
		static const bool emptyValueIsZero = true;
		static void constructDeletedValue(T& slot) {
			new (WTF_NONULL::NotNull, std::addressof(slot)) T(HashTableDeletedValue);
		}
	...
	template<typename T, typename Deleter> struct HashTraits<std::unique_ptr<T, Deleter>> : SimpleClassHashTraits<std::unique_ptr<T, Deleter>> {
		...
		static void constructDeletedValue(std::unique_ptr<T, Deleter>& slot) {
			new (WTF_NONULL::NotNull, std::addressof(slot)) std::unique_ptr<T, Deleter> { reinterpret_cast<T*>(-1) };
		}
		static bool isDeletedValue(const std::unique_ptr<T, Deleter>& value) {
			return value.get() == reinterpret_cast<T*>(-1);
		}

		typedef T* PeekType;
		static T* peek(const std::unique_ptr<T, Deleter>& value) {
			return value.get();
		}
		static T* peek(std::nullptr_t) {
			return nullptr;
		}
	...
	template<typename Traits> struct HashTraitsEmptyValueChecker<Traits, true> {
		template<typename T> static bool isEmptyValue(const T& value) {
	...
	struct TupleHashTraits : GenericHashTraits<std::tuple<typename FirstTrait::TraitType, typename Traits::TraitType...>> {
		...
		static EmptyValueType emptyValue() {
			return std::make_tuple(FirstTrait::emptyValue(), Traits::emptyValue()...);
		}

		static const unsigned minimumTableSize = FirstTrait::minimumTableSize;

		static void constructDeletedValue(TraitType& slot) {
			FirstTrait::constructDeletedValue(std::get<0>(slot));
		}
		static bool isDeletedValue(const TraitType& value) {
			return FirstTrait::isDeletedValue(std::get<0>(value));
		}
	};
	...
	struct KeyValuePairHashTraits : GenericHashTraits<KeyValuePair<typename KeyTraitsArg::TraitType, typename ValueTraitsArg::TraitType>> {
		...
		static EmptyValueType emptyValue() {
			return KeyValuePair<typename KeyTraits::EmptyValueType, typename ValueTraits::EmptyValueType>(KeyTraits::emptyValue(), ValueTraits::emptyValue());
		}

		static const unsigned minimumTableSize = KeyTraits::minimumTableSize;

		static void constructDeletedValue(TraitType& slot) {
			KeyTraits::constructDeletedValue(slot.key);
		}
		static bool isDeletedValue(const TraitType& value) {
...
#define CHECK_HASHTABLE_USE_AFTER_DESTRUCTION 1
	...
	public:
		template<typename T> static unsigned hash(const T& key) {
	...
	public:
		...
		AddResult add(ValueType&& value) {
			return add<IdentityTranslatorType>(Extractor::extract(value), WTFMove(value));
		}
		...
		ValueType* lookup(const Key& key) {
			return lookup<IdentityTranslatorType>(key);
		}
		...
		void internalCheckTableConsistency() const {
			checkTableConsistency();
		}
		void internalCheckTableConsistencyExceptSize() const {
	...
	private:
		...
		bool shouldShrink() const {
			return false;
			return m_keyCount * m_minLoad < m_tableSize&& m_tableSize > KeyTraits::minimumTableSize;
		}
		ValueType* expand(ValueType* entry = nullptr);
		void shrink() {
			rehash(m_tableSize / 2, nullptr);
		}
	...
	void HashTable<Key, Value, Extractor, HashFunctions, Traits, KeyTraits>::checkTableConsistencyExceptSize() const
	{
		...
		ASSERT(m_tableSize == TableSizeMask() + 1);
	}
	...
	struct IdentityExtractor {
		template<typename T> static const T& extract(const T& t) {
			return t;
		}
	};
...
bool codeUnitCompareIgnoringASCIICase(const std::string& a, const std::string& b) {
	auto compareChar = [](char c1, char c2) {
...
std::vector<std::string> process_requestHeaders(std::vector<std::string> extraHeaders, std::vector<std::string> customHeaders, bool isFetch) {
	std::vector<std::string> output;
	if (isFetch) {
		std::sort(customHeaders.begin(), customHeaders.end(), codeUnitCompareIgnoringASCIICase);
		WTF::HashSet<std::string, WTF::StringHash> hashMap = WTF::HashSet<std::string, WTF::StringHash>();
	...
	WTF::HashSet<std::string, WTF::StringHash> hashMap = WTF::HashSet<std::string, WTF::StringHash>();
	WTF::HashSet<std::string, WTF::StringHash> hashMap2 = WTF::HashSet<std::string, WTF::StringHash>();
	WTF::HashSet<std::string, WTF::StringHash> hashMap3 = WTF::HashSet<std::string, WTF::StringHash>();
	for (auto& it : customHeaders) {
		hashMap.add(it);
	}
	/*for (auto& it : hashMap) {
...
std::vector<std::string> process_requestHeadersV2(std::vector<std::string> extraHeaders, std::vector<std::string> customHeaders, bool isFetch) {
	std::vector<std::string> output;
	if (isFetch) {
		std::sort(customHeaders.begin(), customHeaders.end(), codeUnitCompareIgnoringASCIICase);
		WTF::HashSet<std::string, WTF::StringHashV2> hashMap = WTF::HashSet<std::string, WTF::StringHashV2>();
		for (auto& it : customHeaders) {
			hashMap.add(it);
		}
		for (auto& it : extraHeaders) {
	...
	WTF::HashSet<std::string, WTF::StringHashV2> hashMap = WTF::HashSet<std::string, WTF::StringHashV2>();
	WTF::HashSet<std::string, WTF::StringHashV2> hashMap2 = WTF::HashSet<std::string, WTF::StringHashV2>();
	for (auto& it : customHeaders) {
		hashMap.add(it);
	}
	for (auto& it : hashMap) {
