    ...
    std::string& trust_anchors)
{
    ...
    CHECK_CURLOK(curl_easy_setopt(this->m_pCURL, CURLOPT_HTTP3_FP_TRANSPORT_PARAMS, transport_params.c_str()));
...
void BaoCurl::setSSLCert(void* sslCertBuffer, size_t sslCertBufferSize, void* sslPrivateKeyBuffer, size_t sslPrivateKeyBufferSize, std::string& type, std::string& password) {
    struct curl_blob sslCertBlob = { sslCertBuffer, sslCertBufferSize, CURL_BLOB_COPY };
    CHECK_CURLOK(curl_easy_setopt(this->m_pCURL, CURLOPT_SSLCERT_BLOB, &sslCertBlob));
    if (sslPrivateKeyBufferSize != 0) {
        struct curl_blob sslPrivateBlob = { sslPrivateKeyBuffer, sslPrivateKeyBufferSize, CURL_BLOB_COPY };
        CHECK_CURLOK(curl_easy_setopt(this->m_pCURL, CURLOPT_SSLKEY_BLOB, &sslPrivateBlob));
    }
    CHECK_CURLOK(curl_easy_setopt(this->m_pCURL, CURLOPT_SSLCERTTYPE, type.c_str()));
    if (password.size()) {
        CHECK_CURLOK(curl_easy_setopt(this->m_pCURL, CURLOPT_KEYPASSWD, password.c_str()));
    }
}
